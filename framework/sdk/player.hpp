...
public:
	...
	char    szName[260];            //0x0004 
	//OFFSET(char, szName, 0x0004);
	OFFSET(int, eflags, 0xE8);
	OFFSET(animlayer_t*, overlay, 0x2980);
	OFFSET(animstate_t*, animstate, 0x3914);
	OFFSET(CUtlVector< matrix3x4_t >, get_bone_cache, 0x2910); //memmove_0(a3, *(const void **)(v7 + 0x290C + 4), 48 * v90);
	...
	int* get_weapons() {
		return (int*) ((uintptr_t) this + (netvars::get(("DT_CSPlayer"), ("m_hMyWeapons"))));
	}
	...
	std::deque<vec3_t> get_multiple_points_for_hitbox(player_t* local, player_t* entity, int iHitbox, float iHeadScale, float iBodyScale);
