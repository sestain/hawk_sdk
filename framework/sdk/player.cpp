...
void animstate_t::reset() {
	static auto reset_animstate = pattern::search("client.dll", "56 6A 01 68 ? ? ? ? 8B F1").get< void(__thiscall*)(void*) >();
	reset_animstate(this);
}

void animstate_t::update(vec3_t& ang) {
	static auto update_animstate = pattern::search("client.dll", "55 8B EC 83 E4 F8 83 EC 18 56 57 8B F9 F3 0F 11 54").get< void(__vectorcall*)(void*, void*, float, float, float, void*) >();
	update_animstate(this, nullptr, ang.z, ang.y, ang.x, nullptr);
}

bool player_t::physics_run_think(int unk01) {
	static auto func = pattern::search("client.dll", "55 8B EC 83 EC 10 53 56 57 8B F9 8B 87 ? ? ? ? C1 E8 16").get< bool(__thiscall*)(void*, int) >();
	return func(this, unk01);
}

std::uint32_t player_t::handle() {
	using fn = std::uint32_t(__thiscall*)(void*);
	static auto get_ehandle = pattern::search("client.dll", "8B 51 3C 83 FA FF").get< fn >();
	return get_ehandle(this);
}

void player_t::create_animstate(animstate_t* state) {
	static auto create_animstate = pattern::search("client.dll", "55 8B EC 56 8B F1 B9 ? ? ? ? C7 46").get< void(__thiscall*)(void*, void*) >();
	create_animstate(state, this);
}

void player_t::inval_bone_cache() {
	static auto invalidate_bone_cache = pattern::search("client.dll", "80 3D ? ? ? ? ? 74 16 A1 ? ? ? ? 48 C7 81").add(10).get< std::uintptr_t >();
...
void player_t::set_abs_angles(const vec3_t& ang) {
	...
	static auto set_abs_angles = pattern::search("client.dll", "55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1 E8").get< setabsangles_fn >();
	set_abs_angles(this, ang);
}

void player_t::update() {
	using update_clientside_animation_fn = void(__thiscall*)(void*);
	static auto update_clientside_animation = pattern::search("client.dll", "8B F1 80 BE ? ? ? ? 00 74 ? 8B 06 FF 90 ? ? ? ? 8B CE").sub(5).get< update_clientside_animation_fn >();
...
void player_t::set_abs_vel(vec3_t& vel) {
	...
	static auto fn = pattern::search("client.dll", "55 8B EC 83 E4 F8 83 EC 0C 53 56 57 8B 7D 08 8B F1").get< set_abs_vel_fn >();
	fn(this, vel);
}

void player_t::set_abs_origin(vec3_t& vec) {
	using set_abs_origin_fn = void(__thiscall*)(void*, vec3_t&);
	static auto fn = pattern::search("client.dll", "55 8B EC 83 E4 F8 51 53 56 57 8B F1 E8 ? ? ? ? 8B 7D").get< set_abs_origin_fn >();
	fn(this, vec);
}

matrix3x4_t player_t::get_bone_matrix(int BoneID)
{
	matrix3x4_t matrix;
...
vec3_t player_t::eyes() {
	...
	static auto fn = pattern::search("client.dll", "55 8B EC 56 8B 75 08 57 8B F9 56 8B 07 FF 90").get< weapon_shootpos_fn >();
...
std::uint32_t& player_t::bone_count() {
	static auto offset = pattern::search("client.dll", "8B 87 ? ? ? ? 8B 4D 0C").add(2).deref().get< std::uint32_t >();
	return *(uint32_t*) (std::uintptr_t(renderable()) + offset);
}

std::array< matrix3x4_t, 128 >& player_t::bone_cache() {
	static auto offset = pattern::search("client.dll", "FF B7 ? ? ? ? 52").add(2).deref().get< std::uint32_t >();
