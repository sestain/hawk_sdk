...
void render::outline_text(int x, int y, std::uint32_t color, void* font, const std::string_view& text) {
	render::text(x + 0, y - 1, rgba(0, 0, 0, 255), font, text); //up
	render::text(x + 0, y + 1, rgba(0, 0, 0, 255), font, text); //up
	render::text(x + 1, y + 0, rgba(0, 0, 0, 255), font, text); //up
	render::text(x - 1, y + 0, rgba(0, 0, 0, 255), font, text); //up

	render::text(x + 1, y + 1, rgba(0, 0, 0, 255), font, text); //up
	render::text(x - 1, y + 1, rgba(0, 0, 0, 255), font, text); //up

	render::text(x + 1, y - 1, rgba(0, 0, 0, 255), font, text); //up
	render::text(x - 1, y - 1, rgba(0, 0, 0, 255), font, text); //up

	render::text(x + 0, y + 0, color, font, text); //up
}
void render::outline_text(pos p, std::uint32_t color, void* font, const std::string_view& text) {
	auto x = p.x, y = p.y;
...
void render::text_size(void* font, const std::string_view& text, int& w, int& h) {
	auto converted_string = std::wstring(text.begin(), text.end());
	RECT rect = {0, 0, 0, 0};
	reinterpret_cast<ID3DXFont*>(font)->DrawTextW(nullptr, converted_string.data(), converted_string.length(), &rect, DT_CALCRECT, D3DCOLOR_RGBA(0, 0, 0, 0));
	w = rect.right - rect.left;
	h = rect.bottom - rect.top;
}
void render::text_size(void* font, const std::string_view& text, dim& dimensions) {
	auto converted_string = std::wstring(text.begin(), text.end());
	RECT rect = {0, 0, 0, 0};
	reinterpret_cast<ID3DXFont*>(font)->DrawTextW(nullptr, converted_string.data(), converted_string.length(), &rect, DT_CALCRECT, D3DCOLOR_RGBA(0, 0, 0, 0));
	dimensions = dim{rect.right - rect.left, rect.bottom - rect.top};
}

void render::text(int x, int y, std::uint32_t color, void* font, const std::string_view& text) {
	auto converted_string = std::wstring(text.begin(), text.end());
	RECT rect;
	SetRect(&rect, x, y, x, y);
	reinterpret_cast<ID3DXFont*>(font)->DrawTextW(nullptr, converted_string.data(), converted_string.length(), &rect, DT_LEFT | DT_NOCLIP, color);
}
void render::text(pos p, std::uint32_t color, void* font, const std::string_view& text) {
	auto converted_string = std::wstring(text.begin(), text.end());
	RECT rect;
	SetRect(&rect, p.x, p.y, p.x, p.y);
	reinterpret_cast<ID3DXFont*>(font)->DrawTextW(nullptr, converted_string.data(), converted_string.length(), &rect, DT_LEFT | DT_NOCLIP, color);
}
...
void render::rectangle(rect d, std::uint32_t color) {
	vtx_t vert[4] = {
		{d.x - 0.5f, d.y - 0.5f, 0.0f, 1.0f, color},
		{d.x + d.w - 0.5f, d.y - 0.5f, 0.0f, 1.0f, color},
		{d.x - 0.5f, d.y + d.h - 0.5f, 0.0f, 1.0f, color},
		{d.x + d.w - 0.5f, d.y + d.h - 0.5f, 0.0f, 1.0f, color}
	};
...
void render::gradient(rect d, std::uint32_t color1, std::uint32_t color2, bool is_horizontal) {
	const auto d3d_clr1 = color1;
	const auto d3d_clr2 = color2;
	std::uint32_t c1, c2, c3, c4;
	...
	vtx_t vert[4] = {
		{d.x - 0.5f, d.y - 0.5f, 0.0f, 1.0f, c1},
		{d.x + d.w - 0.5f, d.y - 0.5f, 0.0f, 1.0f, c2},
		{d.x - 0.5f, d.y + d.h - 0.5f, 0.0f, 1.0f, c3},
		{d.x + d.w - 0.5f, d.y + d.h - 0.5f, 0.0f, 1.0f, c4}
	};
...
void render::outline(rect d, std::uint32_t color) {
	rectangle(d.x, d.y, d.w, 1, color);
	rectangle(d.x, d.y + d.h, d.w, 1, color);
	rectangle(d.x, d.y, 1, d.h, color);
	rectangle(d.x + d.w, d.y, 1, d.h + 1, color);
}
...
void render::clip_rect(rect d) {
	RECT rect{d.x - 0.5f, d.y - 0.5f, d.x + d.w - 0.5f, d.y + d.h - 0.5f};
	csgo::i::dev->SetScissorRect(&rect);
}
...
void render::line(pos p1, pos p2, std::uint32_t color) {
	vtx_t vert[2] = {
		{p1.x - 0.5f, p1.y - 0.5f, 0.0f, 1.0f, color},
		{p2.x - 0.5f, p2.y - 0.5f, 0.0f, 1.0f, color}
	};
...
void render::circle(pos p, int radius, int segments, std::uint32_t color) {
	std::vector< vtx_t > circle(segments + 2);
	...
	for (auto i = 1; i < segments + 2; i++) {
		circle[i].x = (float) (p.x - radius * std::cosf(D3DX_PI * ((i - 1) / (segments / 2.0f)))) - 0.5f;
		circle[i].y = (float) (p.y - radius * std::sinf(D3DX_PI * ((i - 1) / (segments / 2.0f)))) - 0.5f;
		circle[i].z = 0;
		circle[i].rhw = 1;
		circle[i].color = color;
	}
	...
	for (auto i = 0; i < _res; i++) {
		circle[i].x = p.x + std::cosf(angle) * (circle[i].x - p.x) - std::sinf(angle) * (circle[i].y - p.y) - 0.5f;
		circle[i].y = p.y + std::sinf(angle) * (circle[i].x - p.x) + std::cosf(angle) * (circle[i].y - p.y) - 0.5f;
	}
...
void render::screen_size(dim& d) {
	csgo::i::engine->get_screen_size(d.w, d.h);
}
